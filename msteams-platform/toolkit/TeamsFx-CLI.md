...
|`--capabilities`| No| Choose Teams application capabilities. Options (multiple) are: `tab`, `bot`, `messaging-extension` and `tab-spfx`. The default value is: `tab`.|
...
|`--spfx-framework-type`| No| Applicable if selecting `Tab(SPfx)` capability. Frontend Framework. Options are `none` and `react`, the default value is: `none`.|
|`--spfx-webpart-name`| No | Applicable if selecting `Tab(SPfx)` capability. Webpart Name. The default value is: "helloworld". |
|`--spfx-webpart-desp`| No | Applicable if selecting `Tab(SPfx)` capability. Webpart Description. The default value is: "helloworld description". |
...
#### A Teams tab app in TypeScript with Azure Functions and Azure SQL
